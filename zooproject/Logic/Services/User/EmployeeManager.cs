...
namespace zooproject.Logic.Services.User
{
    public class EmployeeManager
    {
        ...
        {
            return datasource.GetActiveEmployee(activedate);
        }

        public void CreateEmployee(JobType job, Rank rank, DateTime startDate, DateTime? endDate, string firstname, string lastname, Gender usergender, string email, string username, string password, DateTime birthdate, int phoneNumber, int bSN, string specialication,int contract, WorkContract workContract)
        {
            Employee newEmployee = new Employee(job, rank, startDate, endDate, firstname, lastname, phoneNumber, usergender, bSN, email, username, password, birthdate, specialication, contract, workContract);
            datasource.AddEmployee(newEmployee);
        }

        public void CreateRemovedEmployee(Employee employee)
        {
        ...
        public List<Employee> GetEmployeesBySpecialization(Animal animal)
        {
            return datasource.GetEmployeeBySpecialization(animal);
        }

        public List<Employee> GetEmployeesByTaskAndDates(int task, DateTime start, DateTime end)
        {
            return datasource.GetEmployeeByTaskAndDates(task, start, end); 
        }
    }
}