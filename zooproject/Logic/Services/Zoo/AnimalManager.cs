...
namespace zooproject.Logic.Services.Zoo
{
    public class AnimalManager
    {
        ...
        public void CreateAnimal(string name, AnimalSpecies species, DateTime enterdate, string origin, Gender gender, DateTime dateOfBirth, string diet, string notes, string relations, int exhibitID, bool isPredator, bool isPrey, EnviromentType enviromentType, int feedingPeriod, FeedingTimeSlot preferedSlot)
        {
            if (string.IsNullOrEmpty(name)) { throw new DomainException("Name Was Empty"); }
            Animal newAnimal = new Animal(name, species, enterdate, origin, gender, dateOfBirth, diet, notes, relations, exhibitID, isPredator, isPrey, enviromentType, feedingPeriod, preferedSlot);
            datasource.AddAnimal(newAnimal, -1);
        }
        ...
        public void CreateRemovedAnimal(Animal animal, int leavingmode)
        {
            datasource.AddAnimal(animal, leavingmode);
        }
